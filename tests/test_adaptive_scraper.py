...
from services.scraping.adaptive_scraper import AdaptiveWebScraper, create_adaptive_scraper
...
class ScrapingPerformanceTest:
    ...
    async def test_adaptive_queue(self) -> Dict:
        ...
        try:
                ...
                for i, url in enumerate(self.test_urls):
                    ...
                    if processed > 5:  # After initial learning
                        ...
                        if avg_time < 1.0 and workers < 8:
                            workers = min(8, workers + 1)  # Scale up for fast servers
                        elif avg_time > 2.0 and workers > 1:
                            workers = max(1, workers - 1)  # Scale down for slow servers
                    
                    # Process with current worker count
                    if i % workers == 0:
                        await asyncio.sleep(delay / workers)  # Simulate parallel processing
                    ...
                    # Log progress
                    if processed % 10 == 0:
                        print(f"  Progress: {processed}/{len(self.test_urls)} "
            ...
            return {
                ...
                "avg_time_per_url": total_time / len(simulated_results) if simulated_results else 0
            }
    ...
    async def run_comparison(self):
        ...
        if "error" not in static_results and "error" not in adaptive_results:
            speed_improvement = (adaptive_results['urls_per_second'] / 
                               static_results['urls_per_second'])
            time_reduction = ((static_results['total_time'] - adaptive_results['total_time']) / 
                            static_results['total_time'] * 100)
...
async def demonstrate_adaptive_features():
    ...
    print(f"  Target Response Time: {scraper.concurrency_manager.target_response_time}s")
