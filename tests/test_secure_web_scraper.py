...
from services.scraping.web_scraper import (
...
class TestURLSecurityValidator:
    ...
    def test_localhost_blocked(self):
        ...
        for url in localhost_urls:
            valid, msg = URLSecurityValidator.validate_url(url)
            assert valid is False
            assert any(term in msg.lower() for term in ["localhost", "private", "blocked"])
...
class TestSecureWebScraper:
    ...
    async def test_content_size_limit(self, scraper):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            with patch.object(scraper, '_fetch_with_retries', return_value=mock_response):
                with pytest.raises(ValueError) as exc_info:
    ...
    async def test_rate_limiting(self, scraper):
        ...
        with patch('httpx.AsyncClient'), \
             patch.object(scraper, '_fetch_with_retries') as mock_fetch, \
