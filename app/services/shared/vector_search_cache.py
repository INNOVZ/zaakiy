...
import numpy as np
from .cache_service import cache_service, CacheMetrics
...
class VectorSearchCache:
    ...
    async def get_cached_vector_results(
        ...
        try:
            ...
            cached_exact = cache_service.get(exact_key)
            ...
            if cached_exact:
                ...
                logger.debug(
                    "Vector cache EXACT HIT for query: %s", query[:50])
            ...
            if len(embedding) > 0:
                ...
                if similar_result:
                    response_time = (datetime.utcnow() -
                                     start_time).total_seconds() * 1000
                    self.metrics.update_hit(response_time)
                    logger.debug("Vector cache SIMILARITY HIT for query: %s (similarity: %.3f)",
                                 query[:50], similar_result[1])
    ...
    async def cache_vector_results(
        ...
        try:
            ...
            embedding_only_key = f"embedding:v1:{org_id}:{hashlib.md5(query.encode()).hexdigest()}"
            cache_service.set(embedding_only_key, embedding,
                              self.embedding_cache_ttl)

            logger.debug("Cached vector results for query: %s (TTL: %ds, Results: %d)",
                         query[:50], ttl, len(results))
            return True
    ...
    async def get_cached_embedding(self, query: str, org_id: str) -> Optional[List[float]]:
        """Get cached embedding for a query"""
        try:
            embedding_key = f"embedding:v1:{org_id}:{hashlib.md5(query.encode()).hexdigest()}"
            cached_embedding = cache_service.get(embedding_key)
    ...
    def _generate_exact_cache_key(
        ...
        context_config: Dict[str, Any], org_id: str
    ) -> str:
    ...
    def _generate_embedding_cache_key(
        ...
        embedding_str = ','.join(
            [f"{x:.6f}" for x in embedding[:50]])  # First 50 dimensions
    ...
    async def _find_similar_cached_query(
        ...
        try:
            ...
            cached_keys = cache_service.redis_client.keys(
                pattern) if cache_service.enabled else []
    ...
    def _calculate_adaptive_ttl(self, results: List[Dict[str, Any]], search_time_ms: float) -> int:
    ...
    async def _update_popular_queries(self, query: str, org_id: str):
        ...
        try:
            ...
            popular_queries = cache_service.get(key, {})
            ...
            popular_queries[query_hash] = popular_queries.get(
                query_hash, 0) + 1
            ...
            # Cache for 24 hours
            cache_service.set(key, popular_queries, 86400)

        except Exception as e:
            logger.warning("Failed to update popular queries: %s", e)

    async def warm_popular_vectors(self, org_id: str, limit: int = 20) -> Dict[str, Any]:
        ...
        try:
            ...
            for query_hash, frequency in top_queries:
                try:
                    ...
                    existing = cache_service.redis_client.keys(
                        pattern) if cache_service.enabled else []
    ...
    async def invalidate_org_vectors(self, org_id: str) -> int:
        ...
        try:
            ...
            for pattern in patterns:
                ...
                logger.debug("Cleared %d vector cache entries: %s",
                             cleared, pattern)
