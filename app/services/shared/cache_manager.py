...
class CacheManager:
    ...
    async def get_comprehensive_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "error": str(e),
                "recommendations": [
                    "Unable to assess cache status - check system health"
                ],
            }
    ...
    async def optimize_cache_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check memory usage
            memory_usage = cache_health.get("performance_metrics", {}).get(
                "memory_usage_mb", 0
            )
            if memory_usage > 500:  # 500MB threshold
                results["warnings"].append(f"High memory usage: {memory_usage}MB")
            ...
            if pool_usage > 80:
                results["warnings"].append(f"High connection pool usage: {pool_usage}%")
                results["recommendations"].append(
                    "Consider increasing Redis connection pool size"
                )

            # Trigger cache warmup if hit rate is low
            hit_rate = cache_health.get("performance_metrics", {}).get("hit_rate", 0)
            if hit_rate < 60:
                results["recommendations"].append(
    ...
    async def get_cache_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if stats["cache_service"]["circuit_breaker"].get("state") != "CLOSED":
                recommendations.append("Cache circuit breaker is not in normal state")
