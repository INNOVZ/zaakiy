...
from ...config.settings import get_web_scraping_config, get_performance_config
from ...utils.logging_config import get_logger, PerformanceLogger, LogContext
from ...utils.error_handlers import ErrorHandler, handle_errors
from .url_utils import URLSanitizer, log_domain_safely, create_safe_fetch_message, create_safe_success_message, create_safe_error_message
...
class URLSecurityValidator:
    ...
    def validate_url(cls, url: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            localhost_patterns = [
                'localhost', '0.0.0.0', '0x0.0x0.0x0.0x0',
                '0000', '0x0', '127.1', '127.0.1'
            ]
                ...
                try:
                    ...
                    for family, type, proto, canonname, sockaddr in resolved_ips:
                        ...
                        try:
                            ...
                            for private_range in cls.PRIVATE_IP_RANGES:
                                if ip in private_range:
                                    return False, f"Hostname resolves to private IP: {hostname} -> {ip}"
                        except ValueError:
                            continue
...
class RobotsTxtChecker:
    ...
    async def can_fetch(self, url: str, user_agent: str = "*") -> bool:
        ...
        try:
            ...
            logger.warning(f"Error checking robots.txt for {log_domain_safely(url)}: {type(e).__name__}")
...
class SecureWebScraper:
    ...
    async def _fetch_with_retries(self, client: httpx.AsyncClient, url: str) -> httpx.Response:
        ...
        for attempt in range(self.config.max_retries):
            ...
            if attempt < self.config.max_retries - 1:
                ...
                logger.debug(f"Retry {attempt + 1}/{self.config.max_retries} for {log_domain_safely(url)} after {delay:.2f}s")
                await asyncio.sleep(delay)
    ...
    async def scrape_url_text(self, url: str, user_agent: Optional[str] = None) -> str:
        ...
        with LogContext(extra_context={"domain": log_domain_safely(url)}) as ctx:
            logger.info(f"Starting secure scrape of {create_safe_fetch_message(url)}")
            ...
            if self.app_config.blocked_domains and domain in self.app_config.blocked_domains:
                raise ValueError(f"Domain is blocked: {domain}")
            
            if self.app_config.allowed_domains and domain not in self.app_config.allowed_domains:
                raise ValueError(f"Domain not in allowed list: {domain}")
            
            # Check robots.txt compliance
            user_agent_str = user_agent or self._get_random_user_agent()
            if self.app_config.respect_robots_txt and not await self.robots_checker.can_fetch(url, user_agent_str):
            ...
            with PerformanceLogger(f"scrape_url_{domain}"):
                try:
                    async with httpx.AsyncClient(
                        ...
                        logger.info(f"{create_safe_success_message(url, f'{len(text)} characters scraped')}")
                        return text
                        
                except httpx.HTTPStatusError as e:
                    logger.error(f"{create_safe_error_message(url, f'HTTP {e.response.status_code}')}")
                    raise ValueError(f"HTTP {e.response.status_code}: {e.response.reason_phrase}")
                except httpx.TimeoutException:
                    logger.error(f"{create_safe_error_message(url, 'timeout')}")
                    raise ValueError(f"Request timeout after {self.config.timeout}s")
                except Exception as e:
                    logger.error(f"{create_safe_error_message(url, f'unexpected error: {type(e).__name__}')}")
    ...
    async def scrape_website_recursive(
        ...
        with LogContext(extra_context={"domain": log_domain_safely(start_url), "max_pages": max_pages}):
            logger.info(f"Starting recursive scrape: {create_safe_fetch_message(start_url)} (max_pages={max_pages}, max_depth={max_depth})")
            
            # Validate starting URL
            if self.app_config.enable_ssrf_protection:
                is_valid, error_msg = URLSecurityValidator.validate_url(start_url)
                if not is_valid:
                    raise ValueError(f"Start URL security validation failed: {error_msg}")
            ...
            async def scrape_single_url(url: str, depth: int) -> Optional[Tuple[str, str]]:
                ...
                async with semaphore:
                    try:
                        ...
                        logger.warning(f"{create_safe_error_message(url, f'scraping failed: {type(e).__name__}')}")
                        return None
        ...
        while url_queue and len(scraped_pages) < max_pages:
            ...
            if self.app_config.enable_ssrf_protection:
                ...
                if not is_valid:
                    logger.warning(f"Skipping invalid URL {log_domain_safely(current_url)}: {error_msg}")
                    continue
                        ...
                        try:
                            async with httpx.AsyncClient(
                                ...
                                for link in soup.find_all('a', href=True):
                                    ...
                                    is_link_valid, _ = URLSecurityValidator.validate_url(absolute_url)
                                    if not is_link_valid:
                                        continue
                                    
                                    # Domain restriction
                                    if same_domain_only and urlparse(absolute_url).netloc.lower() != start_domain:
                                        continue
                                    
                                    # Add to queue if not visited
                                    if absolute_url not in visited_urls and absolute_url not in [u for u, d in url_queue]:
                                        url_queue.append((absolute_url, depth + 1))
                                        
                        except Exception as e:
                            logger.warning(f"Failed to extract links from {log_domain_safely(current_url)}: {type(e).__name__}")
                            
            except Exception as e:
                logger.error(f"{create_safe_error_message(current_url, f'processing error: {type(e).__name__}')}")
                continue

        logger.info(f"Recursive scrape completed: {len(scraped_pages)} pages scraped")
