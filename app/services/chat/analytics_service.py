...
class AnalyticsService:
    ...
    def __init__(self, org_id: str, supabase_client, context_config):
        self.org_id = org_id
    ...
    async def track_user_feedback(
        ...
        try:
            ...
            if not msg_response.data:
                logger.warning(
                    "Message not found for feedback: %s", message_id)
                return False
            ...
            if response.data:
                try:
                    ...
                    logger.info(
                        "Feedback recorded for message %s: rating=%s", message_id, rating)
    ...
    async def get_performance_insights(self, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            performance_response = self.supabase.table("performance_metrics").select(
                "*"
            ).eq(
                "org_id", self.org_id
            ).gte(
            ...
            success_rate = (successful_operations /
                            total_operations * 100) if total_operations > 0 else 0

            avg_duration = sum(p.get("duration_ms", 0) for p in performance_data) / \
                total_operations if total_operations > 0 else 0

            # Get feedback data
            feedback_response = self.supabase.table("conversation_feedback").select(
                "rating"
            ).eq(
                "org_id", self.org_id
            ).gte(
    ...
    async def track_retrieval_metrics(
        ...
        try:
            ...
            self.supabase.table("retrieval_metrics").insert(
                retrieval_data).execute()

        except Exception as e:
            logger.warning("Retrieval metrics tracking failed: %s", e)

    async def track_context_quality(
        self,
        message_id: str,
        context_length: int,
        ...
        try:
            quality_data = {
                ...
                "overall_quality": (relevance_score + coverage_score + coherence_score) / 3,
                "timestamp": datetime.utcnow().isoformat()
            }

            self.supabase.table("context_quality_metrics").insert(
                quality_data).execute()
    ...
    async def get_analytics_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get message count
            messages_response = self.supabase.table("context_analytics").select(
                "message_id"
            ).eq(
                "org_id", self.org_id
            ).execute()
            ...
            quality_response = self.supabase.table("context_quality_metrics").select(
                "overall_quality"
            ).eq(
                "org_id", self.org_id
            ).gte(
            ...
            avg_quality = sum(q.get("overall_quality", 0)
                              for q in quality_data) / len(quality_data) if quality_data else 0
