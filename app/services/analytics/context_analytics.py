...
# Constants
ANALYTICS_DISABLED_MESSAGE = "Analytics disabled - Supabase not configured"
...
class ContextAnalytics:
    ...
    def log_context_metrics(self, metrics: ContextMetrics) -> bool:
        """Log comprehensive context engineering metrics"""
        if not self.supabase:
            logging.warning(ANALYTICS_DISABLED_MESSAGE)
            return False
        ...
        try:
            ...
            response = (
                self.supabase.table("context_analytics").insert(metrics_data).execute()
            )
    ...
    async def _calculate_dashboard_metrics(
        ...
        avg_context_quality = statistics.mean(context_scores) if context_scores else 0
        avg_satisfaction = (
            ...
            "performance": {
                "response_time": {
                    ...
                    "distribution": self._create_distribution(
                        response_times, [1000, 2000, 3000, 5000]
                    ),
                },
                "context_quality": {
                    "average": round(avg_context_quality, 3),
                    "distribution": self._create_distribution(
                        context_scores, [0.5, 0.7, 0.8, 0.9]
                    ),
                },
                "satisfaction": {
                    ...
                    "distribution": self._create_distribution(
                        satisfaction_scores, [0.3, 0.5, 0.7, 0.9]
                    ),
                },
            },
            "usage_patterns": {
                ...
                "avg_sources_per_query": round(statistics.mean(source_diversity), 2)
                if source_diversity
                else 0,
    ...
    async def _calculate_trends(
        self, current_data: List[Dict], days: int
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            prev_end = (datetime.now(timezone.utc) - timedelta(days=days)).isoformat()
            ...
            return {
                ...
                "response_time_change": self._calculate_percentage_change(
                    prev_avg_response, current_avg_response
                ),
    ...
    def _analyze_quality_patterns(self, data: List[Dict]) -> Dict[str, Any]:
        ...
        # Extract nested conditionals for better readability
        ...
        optimal_source_item = None
        if quality_by_sources:
            optimal_source_item = max(
                quality_by_sources.items(),
                key=lambda x: statistics.mean(x[1]) if x[1] else 0,
            )

        peak_hours = sorted(
            quality_by_time.items(),
            key=lambda x: statistics.mean(x[1]) if x[1] else 0,
            reverse=True,
        )[:3]

        return {
            "best_performing_model": best_model_item[0] if best_model_item else None,
            "optimal_source_count": optimal_source_item[0]
            if optimal_source_item
            else None,
            "peak_quality_hours": peak_hours,
        }
    ...
    def _create_distribution(
        ...
        for i, threshold in enumerate(thresholds):
            if i == 0:
                ...
                count = sum(1 for v in values if thresholds[i - 1] <= v < threshold)
    ...
    def get_query_analysis(
        self, org_id: str, query: str, days: int = 30
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_satisfaction = statistics.mean(
                [
                    q.get("user_satisfaction", 0)
                    for q in similar_queries
                    if q.get("user_satisfaction")
                ]
            )
